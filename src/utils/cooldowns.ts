...
export function keyWordCooldown(keyword: string, cdTime: number) {
  ...
  if (!keywordCD.has(keyword)) {
    keywordCD.set(keyword, cooldownAmount);
  }

  if (keywordCD.has(keyword)) {
    const expireTime = keywordCD.get(keyword) + cooldownAmount;
  ...
  keywordCD.set(keyword, currentTime);
  setTimeout(() => keywordCD.delete(keyword), cooldownAmount);
  return res;
}
